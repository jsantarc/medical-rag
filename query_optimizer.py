"""
optimizer_graph.py — ReAct agent graph with query optimizer node.

Usage:
    from optimizer_graph import _build_graph
"""
...
optimizer_llm = make_obj()

OPTIMIZER_SYSTEM = """You are a clinical search query optimizer.
Return ONLY a single improved search query string using precise medical terminology.
No explanation. No JSON. Just the query string."""


def build_optimizer_prompt(query: str, answer: str) -> str:
...
async def get_improved_query(query: str, answer: str) -> str:
    response = await optimizer_llm.ainvoke([
        SystemMessage(content=OPTIMIZER_SYSTEM),
        HumanMessage(content=build_optimizer_prompt(query, answer))
    ])
